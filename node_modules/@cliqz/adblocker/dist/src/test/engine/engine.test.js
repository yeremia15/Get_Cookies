...
describe('#FiltersEngine', () => {
    ...
    it('cosmetic filters are disabled', () => {
        // Enabled
        expect(Engine.parse('##selector', { loadCosmeticFilters: true }).getCosmeticsFilters({
            domain: 'foo.com',
            hostname: 'foo.com',
            url: 'https://foo.com',
        })).to.eql({
        ...
        // Disabled
        expect(Engine.parse('##selector', { loadCosmeticFilters: false }).getCosmeticsFilters({
            domain: 'foo.com',
            hostname: 'foo.com',
            url: 'https://foo.com',
        })).to.eql({
    ...
    describe('cps policies', () => {
        it('no policy in engine', () => {
            expect(createEngine('this is not a csp').getCSPDirectives(Request.fromRawDetails({
                url: 'https://foo.com',
            }))).to.be.undefined;
        });
        it('network filters are disabled', () => {
            expect(Engine.parse('||foo.com$csp=bar', { loadNetworkFilters: false }).getCSPDirectives(Request.fromRawDetails({
                url: 'https://foo.com',
            }))).to.be.undefined;
        });
        it('request not supported', () => {
            // Not supported protocol
            expect(Engine.parse('||foo.com$csp=bar').getCSPDirectives(Request.fromRawDetails({
                url: 'ftp://foo.com',
            }))).to.be.undefined;
        ...
        it('does not match request', () => {
            expect(createEngine('||bar.com$csp=bar').getCSPDirectives(Request.fromRawDetails({
                url: 'https://foo.com',
            }))).to.be.undefined;
        });
        it('matches request (1 policy)', () => {
            expect(createEngine('||foo.com$csp=bar').getCSPDirectives(Request.fromRawDetails({
                url: 'https://foo.com',
            }))).to.equal('bar');
        });
...
`).getCSPDirectives(Request.fromRawDetails({
...
`).match(request).match).to.be.true;
    ...
    describe('redirect', () => {
        ...
        it('normal redirect', () => {
            ...
            expect(filter.toString()).to.equal('||foo.com$image,redirect=foo.js');
        ...
        it('redirect-rule matches if request was blocked', () => {
            ...
            expect(exception).to.be.undefined;
            expect(filter).not.to.be.undefined;
            expect(filter.toString()).to.equal('||foo.com$image,redirect-rule=foo.js');
            expect(redirect).to.eql({
                body: 'foo.js',
                contentType: 'application/javascript',
                dataUrl: 'data:application/javascript;base64,Zm9vLmpz',
        ...
        it('redirect=none cancels redirect-rule', () => {
            ...
            expect(exception.toString()).to.equal('||foo.com$image,redirect=none');
            expect(filter).not.to.be.undefined;
            expect(filter.toString()).to.equal('||foo.com$image,redirect-rule=foo.js');
            expect(redirect).to.be.undefined;
        });
        ...
        it('exception rule also cancels redirect', () => {
            ...
            expect(match).to.be.false;
            expect(exception).not.to.be.undefined;
            expect(exception.toString()).to.equal('@@||foo.com$image');
            expect(filter).not.to.be.undefined;
            expect(filter.toString()).to.equal('||foo.com$image,redirect=foo.js');
            expect(redirect).to.be.undefined;
        });
        ...
        for (let i = 0; i < requests.length; i += 1) {
            ...
            it(`[request] type=${type} url=${url}, sourceUrl=${sourceUrl}`, () => {
                ...
                for (let j = 0; j < parsedFilters.length; j += 1) {
                    ...
                    test(Object.assign(Object.assign({}, baseConfig), { engine: new Engine({ networkFilters: [filter] }), testFiltersInIsolation: true }));
    ...
    describe('#getCosmeticsFilters', () => {
        describe('script injections', () => {
            ...
            it('script missing', () => {
                expect(Engine.parse('foo.com##+js(foo,arg1)').getCosmeticsFilters({
                    domain: 'foo.com',
                    hostname: 'foo.com',
                    url: 'https://foo.com',
                }).scripts).to.eql([]);
...
`).getCosmeticsFilters({ domain: 'foo.com', hostname: 'foo.com', url: 'https://foo.com' })
        ...
        describe('specifichide', () => {
            it('allows specific cosmetics by default', () => {
                expect(Engine.parse('foo.com##selector').getCosmeticsFilters({
                    domain: 'foo.com',
                    hostname: 'foo.com',
                    url: 'https://foo.com',
                }).styles).not.to.equal('');
...
`).getCosmeticsFilters({ domain: 'foo.com', hostname: 'foo.com', url: 'https://foo.com' })
        ...
        describe('generichide', () => {
            it('allows generic cosmetics by default', () => {
                expect(Engine.parse('##selector').getCosmeticsFilters({
                    domain: 'foo.com',
                    hostname: 'foo.com',
                    url: 'https://foo.com',
                }).styles).not.to.equal('');
