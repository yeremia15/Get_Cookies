...
import { createMap as createPatternMap, deserialize as deserializePattern, getKeys as getPatternKeys, getSerializedSize as getPatternSerializedSize, isValid as isValidPattern, serialize as serializePattern, } from '../../src/engine/metadata/patterns';
import { createMap as createCategoriesMap, deserialize as deserializeCategory, getKey as getCategoryKey, getSerializedSize as getCategorySerializedSize, isValid as isValidCategory, serialize as serializeCategory, } from '../../src/engine/metadata/categories';
import { createMap as createOrganizationsMap, deserialize as deserializeOrganization, getKey as getOrganizationKey, getSerializedSize as getOrganizationSerializedSize, isValid as isValidOrganization, serialize as serializeOrganization, } from '../../src/engine/metadata/organizations';
...
function sortPatterns(pattenrs) {
    return pattenrs.sort((pattern1, pattern2) => fastHash(pattern1.name) - fastHash(pattern2.name));
}
function sortCategories(categories) {
    return categories.sort((category1, category2) => fastHash(category1.name) - fastHash(category2.name));
}
function sortOrganizations(organizations) {
    return organizations.sort((organization1, organization2) => fastHash(organization1.name) - fastHash(organization2.name));
}
describe('#IPattern', () => {
    const view = StaticDataView.allocate(100000, { enableCompression: false });
    it('serialization', () => {
...
describe('#Metadata', () => {
    ...
    describe('#fromFilter', () => {
        ...
        it('retrieves existing metadata', () => {
            expect(metadata.fromFilter(NetworkFilter.parse('||sync.extend.tv^'))).to.eql([
                {
                    pattern: ZYPMEDIA,
                    organization: ZYPMEDIA_ORGANIZATION,
                    category: ADVERTISING_CATEGORY,
        ...
        it('returns undefined otherwise', () => {
            expect(metadata.fromFilter(NetworkFilter.parse('||foo.com^'))).to.eql([]);
        });
    });
    describe('#fromId', () => {
        const metadata = new Metadata({
        ...
        it('retrieves existing metadata', () => {
            var _a;
            expect(metadata.fromId(((_a = NetworkFilter.parse('||sync.extend.tv^')) === null || _a === void 0 ? void 0 : _a.getId()) || 0)).to.eql([
                {
                    pattern: ZYPMEDIA,
                    organization: ZYPMEDIA_ORGANIZATION,
                    category: ADVERTISING_CATEGORY,
        ...
        it('returns undefined otherwise', () => {
            var _a;
            expect(metadata.fromId(((_a = NetworkFilter.parse('||foo.com^')) === null || _a === void 0 ? void 0 : _a.getId()) || 0)).to.eql([]);
        });
    });
    describe('integration with FiltersEngine', () => {
        ...
        it('loads trackerdb dump', () => {
            ...
            if (deserialized.metadata !== undefined) {
                ...
                expect(sortCategories(deserialized.metadata.getCategories())).to.eql(sortCategories(categories));
                expect(sortOrganizations(deserialized.metadata.getOrganizations())).to.eql(sortOrganizations(organizations));
            }
        });
        describe('extends #match with metadata lookup', () => {
            ...
            it('returns metadata', () => {
                expect(engine.match(Request.fromRawDetails({ url: 'https://sync.extend.tv/' }), true /* withMetadata */)).to.eql({
                    exception: undefined,
                    filter,
                    match: true,
                    metadata: [
        ...
        describe('#getPatternMetadata', () => {
            it('matches agaist network filters', () => {
                expect(engine.getPatternMetadata(Request.fromRawDetails({ url: 'https://sync.extend.tv/' }))).to.eql([
                    {
                        'category': ADVERTISING_CATEGORY,
                        'organization': ZYPMEDIA_ORGANIZATION,
                        'pattern': ZYPMEDIA,
            ...
            it('by default does not match against domains', () => {
                expect(engine.getPatternMetadata(Request.fromRawDetails({ url: 'https://extend.tv/' }))).to.eql([]);
            });
            it('matches against domains with getDomainMetadata option', () => {
                expect(engine.getPatternMetadata(Request.fromRawDetails({ url: 'https://sync.extend.tv/' }), {
    ...
    describe('e2e', () => {
        it('loads trackerdb dump', () => {
            ...
            expect(sortCategories(deserializedMetadata.getCategories())).to.eql(sortCategories(categories));
            expect(sortOrganizations(deserializedMetadata.getOrganizations())).to.eql(sortOrganizations(organizations));
