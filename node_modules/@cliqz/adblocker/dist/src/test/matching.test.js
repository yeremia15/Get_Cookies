...
import Request, { getHashesFromLabelsBackward, getHostnameWithoutPublicSuffix, hashHostnameBackward, } from '../src/request';
...
use((chai, utils) => {
    ...
    utils.addMethod(chai.Assertion.prototype, 'matchHostname', function (hostname) {
        const filter = this._obj;
        new chai.Assertion(filter).not.to.be.null;
        this.assert(filter.match(hostname, getDomain(hostname) || ''), 'expected #{this} to match #{exp}', 'expected #{this} to not match #{exp}');
    });
});
...
describe('#getHashesFromLabelsBackward', () => {
    it('hash all labels', () => {
        expect(getHashesFromLabelsBackward('foo.bar.baz', 11, 11)).to.eql(new Uint32Array(['baz', 'bar.baz', 'foo.bar.baz'].map(hashHostnameBackward)));
    });
    it('hash subdomains only', () => {
        expect(getHashesFromLabelsBackward('foo.bar.baz.com', 15, 8 /* start of domain */)).to.eql(new Uint32Array(['baz.com', 'bar.baz.com', 'foo.bar.baz.com'].map(hashHostnameBackward)));
    });
    it('hash ignoring suffix', () => {
        expect(getHashesFromLabelsBackward('foo.bar.baz.com', 11, 11)).to.eql(new Uint32Array(['baz', 'bar.baz', 'foo.bar.baz'].map(hashHostnameBackward)));
    });
    it('hash subdomains only, ignoring suffix', () => {
        expect(getHashesFromLabelsBackward('foo.bar.baz.com', 11, 8)).to.eql(new Uint32Array(['baz', 'bar.baz', 'foo.bar.baz'].map(hashHostnameBackward)));
    });
});
