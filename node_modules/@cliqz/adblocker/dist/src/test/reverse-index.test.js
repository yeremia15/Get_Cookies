...
import { noopOptimizeCosmetic, noopOptimizeNetwork, optimizeNetwork, } from '../src/engine/optimizer';
...
describe('ReverseIndex', () => {
    ...
    [new Config({ enableCompression: true }), new Config({ enableCompression: false })].forEach((config) => {
        describe(`compression = ${config.enableCompression}`, () => {
            describe('#serialize', () => {
                ...
                it('cosmetic', () => {
                    testSerializeIndexImpl(cosmeticFilters, CosmeticFilter.deserialize, noopOptimizeCosmetic);
                });
            });
            describe('#getFilters', () => {
                it('network (optimize = false)', () => {
                    expect(new ReverseIndex({
                        config,
            ...
            for (const optimize of [noopOptimizeNetwork, optimizeNetwork]) {
                describe(`optimize = ${optimize !== noopOptimizeNetwork}`, () => {
                    it('#update', () => {
                        ...
                        // Add one new filter
                        reverseIndex.update(parseFilters('||bar.com', { loadCosmeticFilters: false, debug: true })
                        ...
                        reverseIndex.update(parseFilters('||baz.com', { loadCosmeticFilters: false, debug: true })
...
||foo.com
      `, { loadCosmeticFilters: false, debug: true }).networkFilters,
                            ...
                            [
                                ...
                                // Get all matches
                                matches.clear();
                                index.iterMatchingFilters(tokenize(input, false, false), cb);
                                expect(matches).to.eql(new Set(expected));
                            });
                        });
                    });
...
-bar-
          `).networkFilters,
                                ...
                                .getTokens()
                                .sort()).to.eql(new Uint32Array([fastHash('ads'), fastHash('foo'), fastHash('bar')]).sort());
                        });
                    });
                });
            }
