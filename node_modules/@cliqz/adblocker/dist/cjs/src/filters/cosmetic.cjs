...
var htmlFiltering = require('../html-filtering.cjs');
 ...
 */
const EMPTY_TOKENS = [dataView.EMPTY_UINT32_ARRAY];
...
function computeFilterId(mask, selector, domains, style) {
    ...
    if (selector !== undefined) {
        for (let i = 0; i < selector.length; i += 1) {
            hash = (hash * utils.HASH_INTERNAL_MULT) ^ selector.charCodeAt(i);
        }
    }
    ...
    if (style !== undefined) {
        for (let i = 0; i < style.length; i += 1) {
            hash = (hash * utils.HASH_INTERNAL_MULT) ^ style.charCodeAt(i);
        }
    }
    return hash >>> 0;
}
...
class CosmeticFilter {
    ...
    static parse(line, debug = false) {
        ...
        if (line.length > afterSharpIndex) {
            if (line[afterSharpIndex] === '@') {
                mask = utils.setBit(mask, 1 /* COSMETICS_MASK.unhide */);
                suffixStartIndex += 1;
            }
        ...
        if (sharpIndex > 0) {
            domains$1 = domains.Domains.parse(line.slice(0, sharpIndex).split(','), debug);
        }
        if (line.endsWith(':remove()')) {
            // ##selector:remove()
            mask = utils.setBit(mask, 64 /* COSMETICS_MASK.remove */);
            mask = utils.setBit(mask, 128 /* COSMETICS_MASK.extended */);
            line = line.slice(0, -9);
        }
        ...
        if (line.charCodeAt(suffixStartIndex) === 94 /* '^' */) {
            if (utils.fastStartsWithFrom(line, 'script:has-text(', suffixStartIndex + 1) === false ||
            ...
            if (htmlFiltering.extractHTMLSelectorFromRule(selector) === undefined) {
                return null;
            }
        }
        else if (line.length - suffixStartIndex > 4 &&
            line.charCodeAt(suffixStartIndex) === 43 /* '+' */ &&
            utils.fastStartsWithFrom(line, '+js(', suffixStartIndex)) {
            // Generic scriptlets are invalid, unless they are un-hide
            if ((domains$1 === undefined ||
                (domains$1.hostnames === undefined && domains$1.entities === undefined)) &&
                utils.getBit(mask, 1 /* COSMETICS_MASK.unhide */) === false) {
                return null;
            }
            mask = utils.setBit(mask, 2 /* COSMETICS_MASK.scriptInject */);
            ...
            if (utils.getBit(mask, 1 /* COSMETICS_MASK.unhide */) === false && selector.length === 0) {
                return null;
            }
        }
        else {
            selector = line.slice(suffixStartIndex);
            const selectorType = adblockerExtendedSelectors.classifySelector(selector);
            if (selectorType === adblockerExtendedSelectors.SelectorType.Extended) {
                mask = utils.setBit(mask, 128 /* COSMETICS_MASK.extended */);
            }
            else if (selectorType === adblockerExtendedSelectors.SelectorType.Invalid || !isValidCss(selector)) {
        ...
        if (domains$1 === undefined && utils.getBit(mask, 128 /* COSMETICS_MASK.extended */) === true) {
            return null;
        }
        if (selector !== undefined) {
            // Check if unicode appears in selector
            if (utils.hasUnicode(selector)) {
                mask = utils.setBit(mask, 4 /* COSMETICS_MASK.isUnicode */);
            }
            // Classify selector
            if (utils.getBit(mask, 2 /* COSMETICS_MASK.scriptInject */) === false &&
                utils.getBit(mask, 64 /* COSMETICS_MASK.remove */) === false &&
                utils.getBit(mask, 128 /* COSMETICS_MASK.extended */) === false &&
                ...
                if (utils.getBit(mask, 2 /* COSMETICS_MASK.scriptInject */) === false) {
                    if (c0 === 46 /* '.' */ && isSimpleSelector(selector)) {
                        mask = utils.setBit(mask, 8 /* COSMETICS_MASK.isClassSelector */);
                    }
                    else if (c0 === 35 /* '#' */ && isSimpleSelector(selector)) {
                        mask = utils.setBit(mask, 16 /* COSMETICS_MASK.isIdSelector */);
                    }
                    else if (c0 === 97 /* a */ &&
                        c1 === 91 /* '[' */ &&
                        c2 === 104 /* 'h' */ &&
                        isSimpleHrefSelector(selector, 2)) {
                        mask = utils.setBit(mask, 32 /* COSMETICS_MASK.isHrefSelector */);
                    }
                    else if (c0 === 91 /* '[' */ &&
                        c1 === 104 /* 'h' */ &&
    ...
    static deserialize(buffer) {
        ...
        const isUnicode = utils.getBit(mask, 4 /* COSMETICS_MASK.isUnicode */);
        ...
        return new CosmeticFilter({
            ...
            selector,
            // Optional fields
            domains: (optionalParts & 1) === 1 ? domains.Domains.deserialize(buffer) : undefined,
    ...
    getSerializedSize(compression) {
        ...
        if (this.isUnicode()) {
            estimate += dataView.sizeOfUTF8(this.selector);
        }
        else {
            estimate += dataView.sizeOfCosmeticSelector(this.selector, compression);
        }
        if (this.domains !== undefined) {
            estimate += this.domains.getSerializedSize();
        }
        if (this.rawLine !== undefined) {
            estimate += dataView.sizeOfRawCosmetic(this.rawLine, compression);
        }
        if (this.style !== undefined) {
            estimate += dataView.sizeOfASCII(this.style);
        }
        return estimate;
    }
    /**
    ...
    match(hostname, domain) {
        ...
        if (this.domains !== undefined) {
            ...
            return this.domains.match(hostname.length === 0
                ...
                : request.getHostnameHashesFromLabelsBackward(hostname, domain), hostname.length === 0
                ? dataView.EMPTY_UINT32_ARRAY
                : request.getEntityHashesFromLabelsBackward(hostname, domain));
        }
        return true;
    }
    /**
    ...
    getTokens() {
        ...
        if (tokens.length === 0 && this.isUnhide() === false) {
            if (this.isIdSelector() || this.isClassSelector()) {
                ...
                arr[0] = utils.fastHashBetween(selector, 1, endOfSelector);
                tokens.push(arr);
            }
                ...
                tokens.push(utils.tokenize(this.selector.slice(hrefIndex, hrefEnd), skipFirstToken, skipLastToken));
            }
        }
        if (tokens.length === 0) {
            return EMPTY_TOKENS;
    ...
    getStyleAttributeHash() {
        return `s${utils.fastHash(this.getStyle())}`;
    }
    getSelector() {
        return this.selector;
    }
    getSelectorAST() {
        return adblockerExtendedSelectors.parse(this.getSelector());
    }
    getExtendedSelector() {
        return htmlFiltering.extractHTMLSelectorFromRule(this.selector);
    }
    isExtended() {
        return utils.getBit(this.mask, 128 /* COSMETICS_MASK.extended */);
    }
    isRemove() {
        return utils.getBit(this.mask, 64 /* COSMETICS_MASK.remove */);
    }
    isUnhide() {
        return utils.getBit(this.mask, 1 /* COSMETICS_MASK.unhide */);
    }
    isScriptInject() {
        return utils.getBit(this.mask, 2 /* COSMETICS_MASK.scriptInject */);
    }
    isCSS() {
        return this.isScriptInject() === false;
    }
    isIdSelector() {
        return utils.getBit(this.mask, 16 /* COSMETICS_MASK.isIdSelector */);
    }
    isClassSelector() {
        return utils.getBit(this.mask, 8 /* COSMETICS_MASK.isClassSelector */);
    }
    isHrefSelector() {
        return utils.getBit(this.mask, 32 /* COSMETICS_MASK.isHrefSelector */);
    }
    isUnicode() {
        return utils.getBit(this.mask, 4 /* COSMETICS_MASK.isUnicode */);
    }
    isHtmlFiltering() {
...
exports.DEFAULT_HIDDING_STYLE = DEFAULT_HIDDING_STYLE;
exports.default = CosmeticFilter;
//# sourceMappingURL=cosmetic.cjs.map
