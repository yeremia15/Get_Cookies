...
var domains = require('../engine/domains.cjs');
var dataView = require('../data-view.cjs');
var punycode = require('../punycode.cjs');
 ...
 */
const HTTP_HASH = utils.fastHash('http');
const HTTPS_HASH = utils.fastHash('https');
function isAllowedHostname(ch) {
    return (utils.isDigit(ch) || utils.isAlpha(ch) || ch === 95 /* '_' */ || ch === 45 /* '-' */ || ch === 46 /* '.' */);
}
/**
...
function computeFilterId(csp, mask, filter, hostname, domains, denyallow, redirect) {
    let hash = (utils.HASH_SEED * utils.HASH_INTERNAL_MULT) ^ mask;
    if (csp !== undefined) {
        for (let i = 0; i < csp.length; i += 1) {
            hash = (hash * utils.HASH_INTERNAL_MULT) ^ csp.charCodeAt(i);
        }
    }
    ...
    if (filter !== undefined) {
        for (let i = 0; i < filter.length; i += 1) {
            hash = (hash * utils.HASH_INTERNAL_MULT) ^ filter.charCodeAt(i);
        }
    }
    if (hostname !== undefined) {
        for (let i = 0; i < hostname.length; i += 1) {
            hash = (hash * utils.HASH_INTERNAL_MULT) ^ hostname.charCodeAt(i);
        }
    }
    if (redirect !== undefined) {
        for (let i = 0; i < redirect.length; i += 1) {
            hash = (hash * utils.HASH_INTERNAL_MULT) ^ redirect.charCodeAt(i);
        }
    }
    return hash >>> 0;
}
...
class NetworkFilter {
    static parse(line, debug = false) {
        ...
        if (line.charCodeAt(0) === 64 /* '@' */ && line.charCodeAt(1) === 64 /* '@' */) {
            filterIndexStart += 2;
            mask = utils.setBit(mask, 134217728 /* NETWORK_FILTER_MASK.isException */);
        }
        ...
        if (optionsIndex !== -1 && line.charCodeAt(optionsIndex + 1) !== 47 /* '/' */) {
            ...
            for (const rawOption of line.slice(optionsIndex + 1).split(',')) {
                ...
                switch (option) {
                    case 'denyallow': {
                        denyallow = domains.Domains.parse(optionValue.split('|'), debug);
                        break;
                    }
                    case 'domain':
                    case 'from': {
                        ...
                        domains$1 = domains.Domains.parse(optionValue.split('|'), debug);
                        break;
                    }
                    case 'badfilter':
                        mask = utils.setBit(mask, 131072 /* NETWORK_FILTER_MASK.isBadFilter */);
                        break;
                    case 'important':
                        ...
                        mask = utils.setBit(mask, 1048576 /* NETWORK_FILTER_MASK.isImportant */);
                        break;
                    case 'match-case':
                        // Note: `negation` should always be `false` here.
                        if (negation) {
                    ...
                    case 'third-party':
                        if (negation) {
                            ...
                            mask = utils.clearBit(mask, 32768 /* NETWORK_FILTER_MASK.thirdParty */);
                        }
                        else {
                            // third-party means ~first-party
                            mask = utils.clearBit(mask, 16384 /* NETWORK_FILTER_MASK.firstParty */);
                        }
                        break;
                    case '1p':
                    case 'first-party':
                    ...
                    case 'redirect':
                        ...
                        if (option === 'redirect-rule') {
                            mask = utils.setBit(mask, 536870912 /* NETWORK_FILTER_MASK.isRedirectRule */);
                        }
                    ...
                    case 'csp':
                        ...
                        mask = utils.setBit(mask, 262144 /* NETWORK_FILTER_MASK.isCSP */);
                        if (optionValue.length > 0) {
                            csp = optionValue;
                        }
                        break;
                    ...
                    case 'elemhide':
                        ...
                        mask = utils.setBit(mask, 524288 /* NETWORK_FILTER_MASK.isGenericHide */);
                        mask = utils.setBit(mask, 2097152 /* NETWORK_FILTER_MASK.isSpecificHide */);
                        break;
                    case 'shide':
                    case 'specifichide':
                        if (negation) {
                    ...
                    default: {
                        ...
                        if (negation) {
                            cptMaskNegative = utils.clearBit(cptMaskNegative, optionMask);
                        }
                        else {
                            cptMaskPositive = utils.setBit(cptMaskPositive, optionMask);
                        }
                        break;
                    }
                }
        ...
        if (filterIndexEnd - filterIndexStart >= 2 &&
            ...
            mask = utils.setBit(mask, 4194304 /* NETWORK_FILTER_MASK.isFullRegex */);
        }
        else {
            // Deal with hostname pattern
            if (filterIndexEnd > 0 && line.charCodeAt(filterIndexEnd - 1) === 124 /* '|' */) {
                mask = utils.setBit(mask, 67108864 /* NETWORK_FILTER_MASK.isRightAnchor */);
                filterIndexEnd -= 1;
            }
            if (filterIndexStart < filterIndexEnd &&
                ...
                if (filterIndexStart < filterIndexEnd - 1 &&
                    ...
                    mask = utils.setBit(mask, 268435456 /* NETWORK_FILTER_MASK.isHostnameAnchor */);
                    filterIndexStart += 2;
                }
                else {
                    mask = utils.setBit(mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */);
                    filterIndexStart += 1;
                }
            }
            ...
            if (utils.getBit(mask, 268435456 /* NETWORK_FILTER_MASK.isHostnameAnchor */)) {
                ...
                if (firstSeparator === filterIndexEnd) {
                    ...
                    if (separatorCode === 94 /* '^' */) {
                        ...
                        if (filterIndexEnd - filterIndexStart === 1) {
                            ...
                            mask = utils.setBit(mask, 8388608 /* NETWORK_FILTER_MASK.isRegex */);
                            mask = utils.setBit(mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */);
                        }
                    }
            ...
            if (utils.getBit(mask, 268435456 /* NETWORK_FILTER_MASK.isHostnameAnchor */) === false &&
                filterIndexEnd - filterIndexStart > 0 &&
                line.charCodeAt(filterIndexStart) === 42 /* '*' */) {
                mask = utils.clearBit(mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */);
                filterIndexStart += 1;
            }
            // Transform filters on protocol (http, https, ws)
            if (utils.getBit(mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */)) {
                if (filterIndexEnd - filterIndexStart === 5 &&
                    utils.fastStartsWithFrom(line, 'ws://', filterIndexStart)) {
                    mask = utils.setBit(mask, 4096 /* NETWORK_FILTER_MASK.fromWebsocket */);
                    mask = utils.clearBit(mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */);
                    mask = utils.clearBit(mask, 4 /* NETWORK_FILTER_MASK.fromHttp */);
                    mask = utils.clearBit(mask, 8 /* NETWORK_FILTER_MASK.fromHttps */);
                    filterIndexStart = filterIndexEnd;
                }
                else if (filterIndexEnd - filterIndexStart === 7 &&
                    utils.fastStartsWithFrom(line, 'http://', filterIndexStart)) {
                    mask = utils.setBit(mask, 4 /* NETWORK_FILTER_MASK.fromHttp */);
                    mask = utils.clearBit(mask, 8 /* NETWORK_FILTER_MASK.fromHttps */);
                    mask = utils.clearBit(mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */);
                    filterIndexStart = filterIndexEnd;
                }
                else if (filterIndexEnd - filterIndexStart === 8 &&
                    utils.fastStartsWithFrom(line, 'https://', filterIndexStart)) {
                    mask = utils.setBit(mask, 8 /* NETWORK_FILTER_MASK.fromHttps */);
                    mask = utils.clearBit(mask, 4 /* NETWORK_FILTER_MASK.fromHttp */);
                    mask = utils.clearBit(mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */);
                    filterIndexStart = filterIndexEnd;
                }
            ...
            if (filterIndexEnd - filterIndexStart > 0) {
                ...
                mask = setNetworkMask(mask, 16777216 /* NETWORK_FILTER_MASK.isUnicode */, utils.hasUnicode(filter));
                if (utils.getBit(mask, 8388608 /* NETWORK_FILTER_MASK.isRegex */) === false) {
    ...
    static deserialize(buffer) {
        ...
        const isUnicode = utils.getBit(mask, 16777216 /* NETWORK_FILTER_MASK.isUnicode */);
        ...
        return new NetworkFilter({
            ...
            domains: (optionalParts & 8) === 8 ? domains.Domains.deserialize(buffer) : undefined,
            ...
            denyallow: (optionalParts & 64) === 64 ? domains.Domains.deserialize(buffer) : undefined,
            regex: undefined,
        });
    }
    ...
    getSerializedSize(compression) {
        ...
        if (this.csp !== undefined) {
            estimate += dataView.sizeOfNetworkCSP(this.csp, compression);
        }
        if (this.filter !== undefined) {
            if (this.isUnicode() === true) {
                estimate += dataView.sizeOfUTF8(this.filter);
            }
            else {
                estimate += dataView.sizeOfNetworkFilter(this.filter, compression);
            }
        }
        if (this.hostname !== undefined) {
            estimate += dataView.sizeOfNetworkHostname(this.hostname, compression);
        }
        if (this.domains !== undefined) {
            estimate += this.domains.getSerializedSize();
        }
        if (this.rawLine !== undefined) {
            estimate += dataView.sizeOfRawNetwork(this.rawLine, compression);
        }
        if (this.redirect !== undefined) {
            estimate += dataView.sizeOfNetworkRedirect(this.redirect, compression);
        }
        if (this.denyallow !== undefined) {
    ...
    toString(modifierReplacer) {
        ...
        if (this.fromAny() === false) {
            const numberOfCptOptions = utils.bitCount(this.getCptMask());
            const numberOfNegatedOptions = utils.bitCount(FROM_ANY) - numberOfCptOptions;
    ...
    isRedirectRule() {
        return utils.getBit(this.mask, 536870912 /* NETWORK_FILTER_MASK.isRedirectRule */);
    }
    getRedirect() {
    ...
    getTokens() {
        ...
        if (this.domains !== undefined &&
            ...
            tokensBuffer.TOKENS_BUFFER.push(this.domains.hostnames[0]);
        }
        ...
        if (this.isFullRegex() === false) {
            if (this.filter !== undefined) {
                ...
                utils.tokenizeWithWildcardsInPlace(this.filter, skipFirstToken, skipLastToken, tokensBuffer.TOKENS_BUFFER);
            }
            // Append tokens from hostname, if any
            if (this.hostname !== undefined) {
                utils.tokenizeInPlace(this.hostname, false, this.filter !== undefined && this.filter.charCodeAt(0) === 42 /* '*' */, tokensBuffer.TOKENS_BUFFER);
            }
        }
        else if (this.filter !== undefined) {
            utils.tokenizeRegexInPlace(this.filter, tokensBuffer.TOKENS_BUFFER);
        }
        // If we got no tokens for the filter/hostname part, then we will dispatch
        // this filter in multiple buckets based on the domains option.
        if (tokensBuffer.TOKENS_BUFFER.empty() === true &&
        ...
        if (tokensBuffer.TOKENS_BUFFER.empty() === true && this.fromAny() === false) {
            const types = getListOfRequestTypes(this);
            if (types.length !== 0) {
                const result = [];
                for (const type of types) {
                    const arr = new Uint32Array(1);
                    arr[0] = request.NORMALIZED_TYPE_TOKEN[type];
                    result.push(arr);
                }
                return result;
            }
        ...
        if (this.fromHttp() === true && this.fromHttps() === false) {
            tokensBuffer.TOKENS_BUFFER.push(HTTP_HASH);
        }
        else if (this.fromHttps() === true && this.fromHttp() === false) {
            tokensBuffer.TOKENS_BUFFER.push(HTTPS_HASH);
        }
    ...
    isCptAllowed(cpt) {
        ...
        if (mask !== undefined) {
            return utils.getBit(this.mask, mask);
        }
    ...
    isException() {
        return utils.getBit(this.mask, 134217728 /* NETWORK_FILTER_MASK.isException */);
    }
    isHostnameAnchor() {
        return utils.getBit(this.mask, 268435456 /* NETWORK_FILTER_MASK.isHostnameAnchor */);
    }
    isRightAnchor() {
        return utils.getBit(this.mask, 67108864 /* NETWORK_FILTER_MASK.isRightAnchor */);
    }
    isLeftAnchor() {
        return utils.getBit(this.mask, 33554432 /* NETWORK_FILTER_MASK.isLeftAnchor */);
    }
    isImportant() {
        return utils.getBit(this.mask, 1048576 /* NETWORK_FILTER_MASK.isImportant */);
    }
    isFullRegex() {
        return utils.getBit(this.mask, 4194304 /* NETWORK_FILTER_MASK.isFullRegex */);
    }
    isRegex() {
        return (utils.getBit(this.mask, 8388608 /* NETWORK_FILTER_MASK.isRegex */) ||
            utils.getBit(this.mask, 4194304 /* NETWORK_FILTER_MASK.isFullRegex */));
    }
    isPlain() {
        return !this.isRegex();
    }
    isCSP() {
        return utils.getBit(this.mask, 262144 /* NETWORK_FILTER_MASK.isCSP */);
    }
    isElemHide() {
        return this.isSpecificHide() && this.isGenericHide();
    }
    isSpecificHide() {
        return utils.getBit(this.mask, 2097152 /* NETWORK_FILTER_MASK.isSpecificHide */);
    }
    isGenericHide() {
        return utils.getBit(this.mask, 524288 /* NETWORK_FILTER_MASK.isGenericHide */);
    }
    isBadFilter() {
        return utils.getBit(this.mask, 131072 /* NETWORK_FILTER_MASK.isBadFilter */);
    }
    isUnicode() {
        return utils.getBit(this.mask, 16777216 /* NETWORK_FILTER_MASK.isUnicode */);
    }
    fromAny() {
        return this.getCptMask() === FROM_ANY;
    }
    thirdParty() {
        return utils.getBit(this.mask, 32768 /* NETWORK_FILTER_MASK.thirdParty */);
    }
    firstParty() {
        return utils.getBit(this.mask, 16384 /* NETWORK_FILTER_MASK.firstParty */);
    }
    fromImage() {
        return utils.getBit(this.mask, 16 /* NETWORK_FILTER_MASK.fromImage */);
    }
    fromMedia() {
        return utils.getBit(this.mask, 32 /* NETWORK_FILTER_MASK.fromMedia */);
    }
    fromObject() {
        return utils.getBit(this.mask, 64 /* NETWORK_FILTER_MASK.fromObject */);
    }
    fromOther() {
        return utils.getBit(this.mask, 128 /* NETWORK_FILTER_MASK.fromOther */);
    }
    fromPing() {
        return utils.getBit(this.mask, 256 /* NETWORK_FILTER_MASK.fromPing */);
    }
    fromScript() {
        return utils.getBit(this.mask, 512 /* NETWORK_FILTER_MASK.fromScript */);
    }
    fromStylesheet() {
        return utils.getBit(this.mask, 1024 /* NETWORK_FILTER_MASK.fromStylesheet */);
    }
    fromDocument() {
        return utils.getBit(this.mask, 1 /* NETWORK_FILTER_MASK.fromDocument */);
    }
    fromSubdocument() {
        return utils.getBit(this.mask, 2048 /* NETWORK_FILTER_MASK.fromSubdocument */);
    }
    fromWebsocket() {
        return utils.getBit(this.mask, 4096 /* NETWORK_FILTER_MASK.fromWebsocket */);
    }
    fromHttp() {
        return utils.getBit(this.mask, 4 /* NETWORK_FILTER_MASK.fromHttp */);
    }
    fromHttps() {
        return utils.getBit(this.mask, 8 /* NETWORK_FILTER_MASK.fromHttps */);
    }
    fromXmlHttpRequest() {
        return utils.getBit(this.mask, 8192 /* NETWORK_FILTER_MASK.fromXmlHttpRequest */);
    }
    fromFont() {
        return utils.getBit(this.mask, 2 /* NETWORK_FILTER_MASK.fromFont */);
    }
}
// ---------------------------------------------------------------------------
// Filter parsing
...
function setNetworkMask(mask, m, value) {
    if (value === true) {
        return utils.setBit(mask, m);
    }
    return utils.clearBit(mask, m);
}
/**
...
function checkPattern(filter, request) {
    ...
    if (filter.isHostnameAnchor() === true) {
        ...
        if (filter.isRegex()) {
            ...
            // with nothing in between. So we extract the part of the URL following
            // after hostname and will perform the matching on it.
            return utils.fastStartsWithFrom(request.url, pattern, request.url.indexOf(filterHostname) + filterHostname.length);
        }
        if (filter.hasFilter() === false) {
            return true;
        }
        ...
        // |pattern
        return utils.fastStartsWith(request.url, pattern);
    }
    else if (filter.isRightAnchor()) {
        // pattern|
        return request.url.endsWith(pattern);
