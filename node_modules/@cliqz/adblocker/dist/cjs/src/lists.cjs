...
var network = require('./filters/network.cjs');
var preprocessor = require('./preprocessor.cjs');
...
function detectFilterType(line) {
    ...
    if (firstCharCode === 33 /* '!' */ ||
        ...
        (firstCharCode === 91 /* '[' */ && utils.fastStartsWith(line, '[Adblock'))) {
        return 0 /* FilterType.NOT_SUPPORTED */;
    }
    ...
    if (dollarIndex !== -1 && dollarIndex !== line.length - 1) {
        ...
        if (afterDollarCharCode === 36 /* '$' */ ||
            (afterDollarCharCode === 64 /* '@' */ &&
                utils.fastStartsWithFrom(line, /* $@$ */ '@$', afterDollarIndex))) {
            return 0 /* FilterType.NOT_SUPPORTED */;
        }
    }
    ...
    if (sharpIndex !== -1 && sharpIndex !== line.length - 1) {
        ...
        if (afterSharpCharCode === 35 /* '#'*/ ||
            (afterSharpCharCode === 64 /* '@' */ &&
                utils.fastStartsWithFrom(line, /* #@# */ '@#', afterSharpIndex))
...
function parseFilter(filter) {
    ...
    if (filterType === 1 /* FilterType.NETWORK */) {
        return network.default.parse(filter, true);
    }
    else if (filterType === 2 /* FilterType.COSMETIC */) {
        return cosmetic.default.parse(filter, true);
    }
    return null;
}
function f(strings) {
...
function parseFilters(list, config$1 = new config()) {
    config$1 = new config(config$1);
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (filterType === 1 /* FilterType.NETWORK */ && config$1.loadNetworkFilters === true) {
            const filter = network.default.parse(line, config$1.debug);
            if (filter !== null) {
        ...
        else if (filterType === 2 /* FilterType.COSMETIC */ && config$1.loadCosmeticFilters === true) {
            const filter = cosmetic.default.parse(line, config$1.debug);
            if (filter !== null) {
                if (config$1.loadGenericCosmeticsFilters === true || filter.isGenericHide() === false) {
        ...
        else if (config$1.loadPreprocessors) {
            const preprocessorToken = preprocessor.detectPreprocessor(line);
            if (preprocessorToken === 1 /* PreprocessorTokens.BEGIF */) {
                if (preprocessorStack.length > 0) {
                    preprocessorStack.push(new preprocessor.default({
                        condition: `(${preprocessorStack[preprocessorStack.length - 1].condition})&&(${preprocessor.default.getCondition(line)})`,
                    }));
                }
                else {
                    preprocessorStack.push(preprocessor.default.parse(line));
                }
            }
 ...
 */
function getLinesWithFilters(list, config$1 = new config()) {
    // Set config to `debug` so that we keep track of raw lines for each filter
    return new Set(getFilters(list, new config(Object.assign({}, config$1, { debug: true }))).filters.map(({ rawLine }) => rawLine));
}
/**
 ...
 */
function generateDiff(prevRevision, newRevision, config$1 = new config()) {
    // Set config to `debug` so that we keep track of raw lines for each filter
    const debugConfig = new config(Object.assign({}, config$1, { debug: true }));
