...
function sizeOfUTF8(str) {
    const encodedLength = punycode.encode(str).length;
...
function sizeOfRawNetwork(str, compression) {
    return compression === true
        ? sizeOfBytesWithLength(getCompressionSingleton().networkRaw.getCompressedSize(punycode.encode(str)), false)
        : sizeOfUTF8(str);
}
function sizeOfRawCosmetic(str, compression) {
    return compression === true
        ? sizeOfBytesWithLength(getCompressionSingleton().cosmeticRaw.getCompressedSize(punycode.encode(str)), false)
        : sizeOfUTF8(str);
}
/**
...
class StaticDataView {
    ...
    getUTF8() {
        ...
        this.pos += byteLength;
        return punycode.decode(String.fromCharCode.apply(null, 
        // @ts-ignore
    ...
    pushRawCosmetic(str) {
        if (this.compression !== undefined) {
            this.pushBytes(this.compression.cosmeticRaw.compress(punycode.encode(str)));
        }
        else {
            this.pushUTF8(str);
        }
    ...
    getRawCosmetic() {
        if (this.compression !== undefined) {
            return punycode.decode(this.compression.cosmeticRaw.decompress(this.getBytes()));
        }
        return this.getUTF8();
    }
    pushRawNetwork(str) {
        if (this.compression !== undefined) {
            this.pushBytes(this.compression.networkRaw.compress(punycode.encode(str)));
        }
        else {
            this.pushUTF8(str);
        }
    ...
    getRawNetwork() {
        if (this.compression !== undefined) {
            return punycode.decode(this.compression.networkRaw.decompress(this.getBytes()));
        }
        return this.getUTF8();
    }
    checkSize() {
...
exports.EMPTY_UINT32_ARRAY = EMPTY_UINT32_ARRAY;
exports.EMPTY_UINT8_ARRAY = EMPTY_UINT8_ARRAY;
...
//# sourceMappingURL=data-view.cjs.map
