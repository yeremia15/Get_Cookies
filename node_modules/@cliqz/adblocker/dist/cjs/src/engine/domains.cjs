...
class Domains {
    static parse(parts, debug = false) {
        ...
        for (let hostname of parts) {
            ...
            const hash = request.hashHostnameBackward(negation === true || entity === true ? hostname.slice(start, end) : hostname);
            if (negation) {
                if (entity) {
    ...
    updateId(hash) {
        ...
        if (hostnames !== undefined) {
            for (const hostname of hostnames) {
                hash = (hash * utils.HASH_INTERNAL_MULT) ^ hostname;
            }
        }
        if (entities !== undefined) {
            for (const entity of entities) {
                hash = (hash * utils.HASH_INTERNAL_MULT) ^ entity;
            }
        }
        if (notHostnames !== undefined) {
            for (const notHostname of notHostnames) {
                hash = (hash * utils.HASH_INTERNAL_MULT) ^ notHostname;
            }
        }
        if (notEntities !== undefined) {
            for (const notEntity of notEntities) {
                hash = (hash * utils.HASH_INTERNAL_MULT) ^ notEntity;
            }
        }
        return hash;
    }
    ...
    getSerializedSize() {
        ...
        if (this.entities !== undefined) {
            estimate += dataView.sizeOfUint32Array(this.entities);
        }
        if (this.hostnames !== undefined) {
            estimate += dataView.sizeOfUint32Array(this.hostnames);
        }
        if (this.notHostnames !== undefined) {
            estimate += dataView.sizeOfUint32Array(this.notHostnames);
        }
        if (this.notEntities !== undefined) {
            estimate += dataView.sizeOfUint32Array(this.notEntities);
        }
        if (this.parts !== undefined) {
            estimate += dataView.sizeOfUTF8(this.parts);
        }
        return estimate;
    }
    match(hostnameHashes, entityHashes) {
        ...
        if (this.notHostnames !== undefined) {
            for (const hash of hostnameHashes) {
                if (utils.binLookup(this.notHostnames, hash)) {
                    return false;
                }
            }
        }
        ...
        if (this.notEntities !== undefined) {
            for (const hash of entityHashes) {
                if (utils.binLookup(this.notEntities, hash)) {
                    return false;
                }
            }
        }
        ...
        if (this.hostnames !== undefined || this.entities !== undefined) {
            if (this.hostnames !== undefined) {
                for (const hash of hostnameHashes) {
                    if (utils.binLookup(this.hostnames, hash)) {
                        return true;
                    }
                }
            }
            if (this.entities !== undefined) {
                for (const hash of entityHashes) {
                    if (utils.binLookup(this.entities, hash)) {
                        return true;
                    }
                }
            }
