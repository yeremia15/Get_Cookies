...
var compactSet = require('../../compact-set.cjs');
var cosmetic = require('../../filters/cosmetic.cjs');
var request = require('../../request.cjs');
var utils = require('../../utils.cjs');
var optimizer = require('../optimizer.cjs');
var reverseIndex = require('../reverse-index.cjs');
...
function createStylesheetFromRules(rules) {
    ...
    return createStylesheet(selectors, cosmetic.DEFAULT_HIDDING_STYLE);
}
function createLookupTokens(hostname, domain) {
    const hostnamesHashes = request.getHostnameHashesFromLabelsBackward(hostname, domain);
    const entitiesHashes = request.getEntityHashesFromLabelsBackward(hostname, domain);
...
class CosmeticFilterBucket {
    static deserialize(buffer, config) {
        ...
        bucket.genericRules = filters.deserialize(buffer, cosmetic.default.deserialize, config);
        bucket.classesIndex = reverseIndex.default.deserialize(buffer, cosmetic.default.deserialize, optimizer.noopOptimizeCosmetic, config);
        bucket.hostnameIndex = reverseIndex.default.deserialize(buffer, cosmetic.default.deserialize, optimizer.noopOptimizeCosmetic, config);
        bucket.hrefsIndex = reverseIndex.default.deserialize(buffer, cosmetic.default.deserialize, optimizer.noopOptimizeCosmetic, config);
        bucket.htmlIndex = reverseIndex.default.deserialize(buffer, cosmetic.default.deserialize, optimizer.noopOptimizeCosmetic, config);
        bucket.idsIndex = reverseIndex.default.deserialize(buffer, cosmetic.default.deserialize, optimizer.noopOptimizeCosmetic, config);
        bucket.unhideIndex = reverseIndex.default.deserialize(buffer, cosmetic.default.deserialize, optimizer.noopOptimizeCosmetic, config);
        return bucket;
    }
    constructor({ filters: filters$1 = [], config }) {
        this.genericRules = new filters({
            config,
            deserialize: cosmetic.default.deserialize,
            filters: [],
        });
        this.classesIndex = new reverseIndex.default({
            config,
            deserialize: cosmetic.default.deserialize,
            filters: [],
            optimize: optimizer.noopOptimizeCosmetic,
        });
        this.hostnameIndex = new reverseIndex.default({
            config,
            deserialize: cosmetic.default.deserialize,
            filters: [],
            optimize: optimizer.noopOptimizeCosmetic,
        });
        this.hrefsIndex = new reverseIndex.default({
            config,
            deserialize: cosmetic.default.deserialize,
            filters: [],
            optimize: optimizer.noopOptimizeCosmetic,
        });
        this.htmlIndex = new reverseIndex.default({
            config,
            deserialize: cosmetic.default.deserialize,
            filters: [],
            optimize: optimizer.noopOptimizeCosmetic,
        });
        this.idsIndex = new reverseIndex.default({
            config,
            deserialize: cosmetic.default.deserialize,
            filters: [],
            optimize: optimizer.noopOptimizeCosmetic,
        });
        this.unhideIndex = new reverseIndex.default({
            config,
            deserialize: cosmetic.default.deserialize,
            filters: [],
            optimize: optimizer.noopOptimizeCosmetic,
        ...
        if (filters$1.length !== 0) {
            this.update(filters$1, undefined, config);
        }
    }
    getFilters() {
        const filters = [];
    ...
    getBaseRules = true, getInjectionRules = true, getExtendedRules = true, getRulesFromDOM = true, getRulesFromHostname = true, isFilterExcluded, }) {
        ...
        if (allowGenericHides === true && getRulesFromDOM === true && classes.length !== 0) {
            this.classesIndex.iterMatchingFilters(utils.hashStrings(classes), (rule) => {
                if (rule.match(hostname, domain) && !(isFilterExcluded === null || isFilterExcluded === void 0 ? void 0 : isFilterExcluded(rule))) {
                    rules.push(rule);
                }
                return true;
        ...
        if (allowGenericHides === true && getRulesFromDOM === true && ids.length !== 0) {
            this.idsIndex.iterMatchingFilters(utils.hashStrings(ids), (rule) => {
                if (rule.match(hostname, domain) && !(isFilterExcluded === null || isFilterExcluded === void 0 ? void 0 : isFilterExcluded(rule))) {
                    rules.push(rule);
                }
                return true;
        ...
        if (allowGenericHides === true && getRulesFromDOM === true && hrefs.length !== 0) {
            this.hrefsIndex.iterMatchingFilters(compactSet.compactTokens(compactSet.concatTypedArrays(hrefs.map((href) => utils.tokenizeNoSkip(href)))), (rule) => {
                if (rule.match(hostname, domain) && !(isFilterExcluded === null || isFilterExcluded === void 0 ? void 0 : isFilterExcluded(rule))) {
                    rules.push(rule);
                }
                return true;
