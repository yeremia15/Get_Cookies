...
class FiltersContainer {
    ...
    update(newFilters, removedFilters) {
        ...
        if (selected.length === 0) {
            ...
            const buffer = dataView.StaticDataView.allocate(bufferSizeEstimation, this.config);
    ...
    getSerializedSize() {
        return dataView.sizeOfBytes(this.filters, false /* no alignement */);
    }
    serialize(buffer) {
    ...
    getFilters() {
        ...
        const filters = [];
        const buffer = dataView.StaticDataView.fromUint8Array(this.filters, this.config);
