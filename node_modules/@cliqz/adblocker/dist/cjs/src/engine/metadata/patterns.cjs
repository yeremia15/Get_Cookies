...
function getKeys(pattern) {
    ...
    for (const filter of pattern.filters) {
        const parsedFilter = network.default.parse(filter);
        if (parsedFilter !== null) {
            keys.push(parsedFilter.getId());
        }
    }
    for (const domain of pattern.domains) {
        const parsedFilter = network.default.parse(`||${domain}^`);
        if (parsedFilter !== null) {
            keys.push(parsedFilter.getId());
        }
    }
...
function getSerializedSize(pattern) {
    let sizeOfDomains = dataView.sizeOfLength(pattern.domains.length);
    for (const domain of pattern.domains) {
        sizeOfDomains += dataView.sizeOfUTF8(domain);
    }
    let sizeOfFilters = dataView.sizeOfLength(pattern.filters.length);
    for (const filter of pattern.filters) {
        sizeOfFilters += dataView.sizeOfUTF8(filter);
    }
    return (dataView.sizeOfUTF8(pattern.key) +
        dataView.sizeOfUTF8(pattern.name) +
        dataView.sizeOfUTF8(pattern.category) +
        dataView.sizeOfUTF8(pattern.organization || '') +
        dataView.sizeOfUTF8(pattern.alias || '') +
        dataView.sizeOfUTF8(pattern.website_url || '') +
        dataView.sizeOfUTF8(pattern.ghostery_id || '') +
        sizeOfDomains +
        sizeOfFilters);
}
