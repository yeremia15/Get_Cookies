...
class ReverseIndex {
    static deserialize(buffer, deserialize, optimize, config) {
        ...
        const view = dataView.StaticDataView.fromUint8Array(buffer.getBytes(true /* align */), config);
    ...
    constructor({ deserialize, filters, optimize, config, }) {
        ...
        this.cache = new Map();
        this.view = dataView.StaticDataView.empty(config);
    ...
    update(newFilters, removedFilters) {
        ...
        if (filters.length === 0) {
            this.updateInternals({
                ...
                view: dataView.StaticDataView.empty(this.config),
            });
            return;
        }
        ...
        const buffer = dataView.StaticDataView.allocate(estimatedBufferSize, this.config);
