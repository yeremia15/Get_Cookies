...
class CompactMap {
    static deserialize(buffer, deserialize) {
        ...
        const view = dataView.StaticDataView.fromUint8Array(buffer.getBytes(true /* align */), {
    ...
    constructor({ serialize, deserialize, getKeys, getSerializedSize, values, }) {
        ...
        this.cache = new Map();
        this.bucketsIndex = dataView.EMPTY_UINT32_ARRAY;
        this.tokensLookupIndex = dataView.EMPTY_UINT32_ARRAY;
        this.valuesIndexStart = 0;
        this.numberOfValues = 0;
        this.view = dataView.StaticDataView.empty({ enableCompression: false });
        ...
        if (values.length !== 0) {
            ...
            if (values.length === 0) {
                this.updateInternals({
                    bucketsIndex: dataView.EMPTY_UINT32_ARRAY,
                    valuesIndexStart: 0,
                    numberOfValues: 0,
                    tokensLookupIndex: dataView.EMPTY_UINT32_ARRAY,
                    view: dataView.StaticDataView.empty({ enableCompression: false }),
                });
                return;
            }
            for (const value of values) {
            ...
            const tokensLookupIndexSize = Math.max(2, reverseIndex.nextPow2(values.length));
            ...
            const buffer = dataView.StaticDataView.allocate(estimatedBufferSize, { enableCompression: false });
    ...
    getSerializedSize() {
        ...
        return 12 + dataView.sizeOfBytes(this.view.buffer, true /* align */);
    }
    /**
