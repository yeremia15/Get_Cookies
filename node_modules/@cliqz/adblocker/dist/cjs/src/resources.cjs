...
var small = require('@remusao/small');
...
class Resources {
    ...
    getResource(name) {
        const { body, contentType } = this.resources.get(name) || small.getResourceForMime(name);
        let dataUrl;
    ...
    getSerializedSize() {
        let estimatedSize = dataView.sizeOfASCII(this.checksum) + 2 * dataView.sizeOfByte(); // resources.size
        this.resources.forEach(({ contentType, body }, name) => {
            estimatedSize += dataView.sizeOfASCII(name) + dataView.sizeOfASCII(contentType) + dataView.sizeOfUTF8(body);
        });
